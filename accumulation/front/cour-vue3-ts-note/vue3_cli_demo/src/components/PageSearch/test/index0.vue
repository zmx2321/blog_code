<template>
  <section>测试表单按钮</section>
  <section class="search-form-box">
    <el-form :model="formData" ref="formRef" :inline="true" class="f-wrap">
      <template v-for="(item, index) in formOptions">
        ...
        >
          ...
          <form-item v-else v-model="formData[item.prop]" :itemOptions="item" />
        </el-form-item>
      </template>
      ...
      <div class="btn-box mb16">
        ...
        >
        <el-button
          v-if="btnItems.includes('inquire')"
          class="secondary-btn"
          @click="onInquire"
          >查询</el-button
        >
