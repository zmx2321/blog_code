<template>
  <div class="header" :style="{ height: settingStore.getHeaderHeight + 'px' }" flex="main:justify cross:center">
    <div class="left">
      <app-icon
        icon="hl-zhediemianban"
        color="#333333"
        class="ft24 pointer animate"
        :class="!settingStore.getIsCollapseMenu && 'turn-rotate'"
        @click="collapseMenu"></app-icon>
    </div>
    <div class="right">
      <el-icon class="mr24 pointer" size="18" v-if="settingStore.showSearch" @click="settingStore.showPageSearch = true"><Search /></el-icon>
      <el-icon class="mr24 pointer" size="18" v-if="settingStore.showFullScreen" @click="fullScreenClick"><FullScreen /></el-icon>
      <el-icon class="pointer" size="18" v-if="settingStore.showTheme" @click="editThemeClick"><Brush /></el-icon>
    </div>
    <el-dialog v-model="settingStore.showPageSearch" title="Inner Dialog" />
    <!-- 主题修改抽屉 -->
...
import { Search, FullScreen, Brush } from '@element-plus/icons-vue'
