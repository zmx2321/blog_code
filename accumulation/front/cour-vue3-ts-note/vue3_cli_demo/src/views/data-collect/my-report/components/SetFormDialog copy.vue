  ...
  <el-dialog v-model="dialogVisible" title="表单设置" width="1200px" lock-scroll :close-on-click-modal="false">
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="nextSubmit(formRef)">下一步</el-button>
      </span>
    </template>
  </el-dialog>
...
let params = {
  ...
  // 报表基本参数 - 必填
  report: {
    audit: 1, // 审批标识 0关闭，1启用   必填
    reportName: '11', // 表单名称
    tableName: 'test', // 数据库名称
    enabled: true // 开启状态 - 页面上没有 - 默认true
  },
  // 报表具体数据
  reportSchedule: {
    distributeType: '1', // 分发类型 1:定时分发 ,2:只分发一次 - 必填
    collectObjType: '2', // 收集对象类型1, "组织"  2, "个人" - 必填  树只选人
    frequency: 1, // 执行频率 0 未设置 1:年,2:月,3:日 - 必填  distributeType = 1时必填
    distributeTime: '20：15', // 分发时间 - distributeType = 1时必填
    distributeDay: '1', // 分发时间 - distributeType = 1时必填
    distributeMonth: '2', // 分发时间 - distributeType = 1时必填
    ...
    collectObj: [20019880, 20019879] // collectObjType = 2时只传数组 必填
  }
}
 ...
 */
// 打开弹窗
const openCreateReportDialog = (row) => {
